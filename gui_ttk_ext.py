...
def bring_to_top( root_frame  ):
    ...
    gui_ttk_ext.bring_to_top( self.root )  # in polling0
    """
...
def set_icon( root_frame, icon_fn  ):
    ...
    try:
        ...
        print( f"icon failed for file = >{icon_fn}<")

    if False:   #self.parameters.os_win:
        import ctypes
        myappid = icon_fn # arbitrary string
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID( myappid )
        #rint( "in windows setting icon" )
        ## surround with try except
        root_frame.iconbitmap( icon_fn )
...
def make_root( parameters  ):
    ...
    if  parameters.gui_theme_type  in ["none", "ttk" ]:
        ...
        #rint( f"available themes are:  {root.pixmap_themes}" )
...
class FileBrowseWidget( tk.Frame ):
    ...
    need an example interface to this
        ...
        a_bw               = gui_ttk_ext.FileBrowseWidget( self.gui.root )
        a_bw.initialdir    = "./",
        a_bw.title         = "Select picture file",
        a_bw.filetypes     = (("Text file","*.txt"),("all files","*.*"))
    ...
    build it and keep a reference
        a_bw     = gui_ttk_ext.FileBrowseWidget( a_parent )  self.gui.root ?
    get the text from it
        ret    = a_bw.get_text()     and set_text()
...
#        color   = "black"   # this may need a bit of rework -- looks like not used
    ...
    def do_copy_button( self, event ):
        ...
        copy All text to the clipboard
        """
        print( "do_copy_button -- this is all text ")
        data  = self.msg_text.get( 1.0, tk.END )
        pyperclip.copy( data )
    ...
    def copy_selection( self, ):
        ...
        !! still needs to be changed to clipper ??? get
        instance from app golobal ??
...
class PlaceInGrid( ):
    ...
    not clear need by row mode also not finished

    placer    = gui_ttk_ext.PlaceInGrid( 99,  by_rows = False )
...
#    add a span argument
    ...
    # -----------------------------------
    def new_row( self, delta_row = 1, indent = -1 ):
        """
        start a new row in col 0
        !! also for col
        """
        if indent != -1:
            print( "placer new_row do we need an enhancement")
